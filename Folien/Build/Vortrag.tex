...
\subsection{Direkte und indirekte CP-Verletzung}
  ...
  \begin{frame}{Was wird denn da gemessen?}
    \begin{columns}[onlytextwidth]
      ...
      \begin{column}{0.5\textwidth}
        \begin{itemize}
          \item Vorteil: Viele systematische Fehler kürzen sich raus
          \item Wäre $\epsilon^{\prime} = 0$ so gäbe es keine direkte CP-Verletzung
        ...
        \begin{itemize}
          \item Es sollten somit drei Generationen gemessen werden können
          \item Direkte CP-Verletzung sollte bei Kaonen (und in anderen Systemen $\rightarrow$ B-Mesonen) gemessen werden
        \end{itemize}
      \end{column}
    \end{columns}
  \end{frame}
